...
{ + инициализация массива функций и таблицы замещений.                         }
{ Использовать только на процессорах с автосинхронизацией (486DX и выше):      }
{ отсутствует FWAIT.                                                           }
{                             ver 3.4.18                                       }
{******************************************************************************}
...
//коды комманд должны начинаться с 1 и последовательно возрастать (+1) !!!
//коды C_IPWR3-C_IPWR16;C_IPWR3N-C_IPWR16N должны идти последовательно(+1 на последующий)!!!
 ...
 //LExternalFuncList: Integer = 38;          //внешние ф-ии;(прописанные в ядре (поиск имён))
...
i: Integer;
begin
//список внутренних (исполняемых) функций
...
{
Ячейка таблицы:
|---------|     WCS - ранг  сохранения (какая часть команды (Func1) сохраняется)
| `  WCS  |     RCS - приоритет чтения (какая часть команды (Func2) замещается)
|   `     |     NC  - число сохраняемых частей за раз
| RCS `   |
|-------- |
...
{XchCode[Number of Length of  XchTable] = Number Function in FuncList}
 ...
 //////частичная замена отключена из-за ошибки:  sh(x)/(-exp(x))+ch(x)/(-sh(x))

 //гиперб. ф-ии: S=exp;
 //для sh,ch (соответственно):
 //(RCS: 1 - (sh,ch); 2 - (exp,1/exp); 3 - (exp))
 //(WCS: -//-)
 //для th,cth (соответственно):
...
asm
   ...
   @@2: shr     eax,1
        ...
        {fld1
        fdivrp}
        //возникает странная ошибка: результат "прыгает", например: sin((3*x+4)^(2*i-1)) [x=2,i=5]
        //для этого добавляются wait
   @@3:
         //POP     EDX
         //POP     ECX
...
//используется только в Lib (flGetAddress)
